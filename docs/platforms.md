...
- **AutoGen v4.0 (Microsoft Research)** — Open-source multi-agent framework with hierarchical agent orchestration and real-time collaboration via WebSockets. Scalable multi-agent reasoning at 1M tokens/sec, 3x faster on complex tasks (SOTA on GAIA benchmark), 40% efficiency gains in task decomposition. GitHub stars: 15k+.
