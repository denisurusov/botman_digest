...
- **AutoGen v3.5 (Microsoft Research)** — Open-source multi-agent framework with hierarchical multi-agent orchestration, improved tool-calling, 40% faster execution, supports hybrid human-AI teams. Includes hierarchical agent swarms, self-healing loops/agents, quantum simulator integration, 10+ new templates for robotics swarms/code review/market analysis, and real-time collaboration via WebSockets. Scalable multi-agent reasoning at 1M tokens/sec, 2x better task completion and 3x faster on complex tasks (SOTA on GAIA benchmark), 40% efficiency gains in task decomposition. Enhances multi-agent collaboration with dynamic role-switching and built-in conflict resolution for complex task decomposition; 25% better performance on GAIA benchmark. GitHub stars: 50k+.
...
## Frontier Labs
- **Cowork 2.0** — Major update to enterprise LLM orchestration framework, adding native support for federated learning across hybrid clouds. Includes 40% faster inference for RAG pipelines.
...
## Hugging Face
- **Agents Hub** — Open platform for sharing/deploying AI agents, with 500+ pre-built agents from community. Integrates with Transformers library for rapid enterprise agent development and deployment.
