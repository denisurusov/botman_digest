...
- **AutoGen v3.0 (Microsoft Research)** — Open-source multi-agent framework supporting dynamic agent hierarchies and real-time collaboration via WebSockets. Hierarchical agent orchestration with scalable multi-agent reasoning at 1M tokens/sec, 3x faster on complex tasks (SOTA on GAIA benchmark, 40% improvement in task completion).
